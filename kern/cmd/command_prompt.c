...
int process_command(int number_of_arguments, char** arguments)
{
	...
	LIST_INIT(&foundCommands);
		//loop to iterate over all predefined commands
		for (int command_no = 0; command_no < NUM_OF_COMMANDS; command_no++)
		{
			//assuming that the current command is valid
			bool valid = 1;
			//if the length of the given command not equal in length of the entered command (so it's not valid)
			if(strlen(commands[command_no].name) != strlen(arguments[0])){
				valid = 0;
			}
			//loop iterate over every character in the given command name and compare it to the entered if they match (then increment j to go to the next character else it would be invalid)
			int counter = 0;
			for(int character_no = 0; character_no < strlen(commands[command_no].name);character_no++){
					if(commands[command_no].name[character_no] == arguments[0][counter]){
						counter++;
					}
					else{
						valid = 0;
					}
					//if the length of j is equal to the length of entered command name then put the command in the found list and exit the loop.
					if(counter == strlen(arguments[0])){
					    LIST_INSERT_TAIL(&foundCommands, &commands[command_no]);
						break;
					}
			}
			    //if it is valid
				if(valid && counter == strlen(arguments[0])){
					//the nested loop check if the number of arguments of the given match with the entered)
					if(commands[command_no].num_of_args == number_of_arguments - 1 || (commands[command_no].num_of_args == -1 && number_of_arguments >=2)){
						LIST_INIT(&foundCommands);
						return command_no;
					}
					//else initialize the list and insert the found command in the list then return invalid number of arguments.
					else{
						LIST_INIT(&foundCommands);
						LIST_INSERT_TAIL(&foundCommands, &commands[command_no]);
		...
		//if the list wasn't empty then we will return to matched commands (which is a list that contains all matched commands).
		if(LIST_SIZE(&foundCommands) > 0){
			return CMD_MATCHED;
		}
		//else it's invalid command.
		return CMD_INVALID;
}
