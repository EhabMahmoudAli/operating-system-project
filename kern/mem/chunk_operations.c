...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 endVa = virtual_address + size;

	for (uint32 i = virtual_address; i < endVa; i += PAGE_SIZE){
		get_page_table(e->env_page_directory, i, &ptr_page_table);

		if (ptr_page_table == NULL){
			create_page_table(e->env_page_directory, i);
		}
		pt_set_page_permissions(e->env_page_directory, i, PERM_AVAILABLE, 0); //mark as unavailable
	}
}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for (uint32 i = virtual_address; i < endVa; i += PAGE_SIZE){
		for(uint32 j = 0; j < e->index1; j++){
			if (e->arr1[j].va == i){
				e->arr1[j].va = 0;
				e->arr1[j].size = 0;
			}
		}
		pt_set_page_permissions(e->env_page_directory, i, 0, PERM_AVAILABLE); // mark as available
		pf_remove_env_page(e, i);
		env_page_ws_invalidate(e, i);
		unmap_frame(e->env_page_directory,i);
	}

	// To maintain correct FIFO order:
