...
uint32 get_hard_limit(){
	return (uint32)curenv->hard_limit;
}

uint32* get_page_table_fun(uint32 virtual_address, uint32* ptr){
	get_page_table(ptr_page_directory, virtual_address, &ptr);
	return ptr;
}

uint32 get_permissions(uint32 virtual_address){
	return (uint32)pt_get_page_permissions(curenv->env_page_directory, virtual_address);
}

void set_permissions(uint32 virtual_address){
	pt_set_page_permissions(curenv->env_page_directory, virtual_address, PERM_AVAILABLE, 0);
}

void update_arr(uint32 virtual_address, uint32 size){
	int flag = 0;
	for (uint32 i = 0; i < curenv->index1; i++){
		if (curenv->arr1[i].va == 0){
			curenv->arr1[i].va = virtual_address;
			curenv->arr1[i].size = size;
			flag = 1;
		}
	}
	if (!flag){
		curenv->arr1[curenv->index1].va = virtual_address;
		curenv->arr1[curenv->index1].size = size;
		curenv->index1++;
	}
}

uint32 get_size(uint32 virtual_address){
	for (uint32 i = 0; i < curenv->index1; i++){
		if (curenv->arr1[i].va == virtual_address){
			return curenv->arr1[i].size;
		}
	}
	return 0;
}
...
void* sys_sbrk(int increment)
{
	...
	uint32 brk_value =env->segment_break; // pointer points to its place of break
	if ((brk_value + increment) >= env->hard_limit || (brk_value + increment) < env->start){
//		panic("NOT ENOUGH MEMORY!!!\n");
		cprintf("NOT ENOUGH MEMORY!!!\n");
	}
	if(increment > 0){
		if((env->segment_break + (uint32)increment) < env->hard_limit){
			uint32* ptr_page_table ;
			int ret = get_page_table(env->env_page_directory, env->segment_break, &ptr_page_table);
			if(ret == TABLE_NOT_EXIST){
				create_page_table(env->env_page_directory, env->segment_break);
			}
			pt_set_page_permissions(env->env_page_directory,env->segment_break, PERM_AVAILABLE, 0);
			uint32 numOfFrames = (ROUNDUP(increment,PAGE_SIZE))/PAGE_SIZE;
			if(increment + env->segment_break < ROUNDUP(env->segment_break,PAGE_SIZE)){
				env->segment_break = ROUNDUP(env->segment_break,PAGE_SIZE);
				return (void*)brk_value;
			}
			else if(ROUNDUP(env->segment_break,PAGE_SIZE)-env->segment_break >= increment-PAGE_SIZE){
			...
			return (void*)brk_value;
		} // return the previous position break
				return (void*)-1;
	}
	else if(increment == 0){
		return (void*)brk_value;
		...
		incr1 -= increment;
		uint32 numOfFrames = (ROUNDDOWN(incr1,PAGE_SIZE))/PAGE_SIZE;
		uint32 x = env->segment_break;
		if(numOfFrames >= 1){
			uint32 x = (ROUNDUP(env->segment_break,PAGE_SIZE));
		}
		for(uint32 i=0;i<numOfFrames;i++){
			pt_set_page_permissions(env->env_page_directory,ROUNDUP(x,PAGE_SIZE), 0, PERM_AVAILABLE); //mark as unavailable
			env_page_ws_invalidate(env,ROUNDUP(x,PAGE_SIZE));
			unmap_frame(env->env_page_directory,ROUNDUP(x,PAGE_SIZE));
			pf_remove_env_page(env,ROUNDUP(x,PAGE_SIZE));
			x+=PAGE_SIZE;
			incr1-=PAGE_SIZE;
		}
		struct WorkingSetElement *ptr_WS_element;
		struct WorkingSetElement *ptr_WS_element_temp;
		struct WorkingSetElement *ptr_WS_element_save;

		LIST_FOREACH(ptr_WS_element, &(env->page_WS_list)){
			if(ptr_WS_element->prev_next_info.le_next == NULL){
				break;
			}
		}
		...
		//ptr_WS_element_temp = ptr_WS_element;
		env_page_ws_print(curenv);
		while(ptr_WS_element_temp != env->page_last_WS_element){
			ptr_WS_element_temp = ptr_WS_element->prev_next_info.le_prev;
			LIST_REMOVE(&env->page_WS_list,ptr_WS_element);
			LIST_INSERT_HEAD(&env->page_WS_list,ptr_WS_element);
			ptr_WS_element = ptr_WS_element_temp;
		}
		if(env->page_last_WS_element!=NULL){
			LIST_REMOVE(&env->page_WS_list,env->page_last_WS_element);
			LIST_INSERT_HEAD(&env->page_WS_list,env->page_last_WS_element);
		}
		return (void*)env->segment_break;
	}
	return (void*)-1;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_allocate_user_mem:
		if ((uint32*)a1 == NULL || (uint32*)a2 == NULL || ((uint32*)(a1) + a2 > (uint32*)USER_LIMIT)){
	...
	case SYS_get_page_table:
		return (uint32)get_page_table_fun(a1, (uint32*)a2);
		break;

	case SYS_get_permissions:
		return (uint32) get_permissions(a1);
		break;

	case SYS_set_permissions:
		set_permissions(a1);
		break;
	...
	case SYS_get_size:
		return (uint32)get_size(a1);
		break;

	case SYS_env_set_nice:
		sys_env_set_nice((struct Env*)(a1), (uint32)(a2));
		break;
	//=====================================================================
	case SYS_cputs:
